...
card : {attributes:List (Attribute msg),selected:Bool,onPress : Maybe msg,content:Element msg}
    -> Card msg
...
hand attributes {scale,dimensions,width,cards}=
  let
    cardsAmount : Float
    cardsAmount = (cards |> List.length |> toFloat)


    (cardWidth,cardHeight) = dimensions
    spacing =
      if cardsAmount-1 <= 0 then
        0
      else
        (width - scale*cardsAmount*cardWidth)/(cardsAmount-1)
